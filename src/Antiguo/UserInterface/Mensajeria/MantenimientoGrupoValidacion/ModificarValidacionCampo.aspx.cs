...
{
    public partial class ModificarValidacionCampo : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                VALIDACION_CAMPO vc = BusinessLayer.Mensajeria.ValidacionCampoBL.obtenerValidacionCampoPorCodigo(Convert.ToInt32(Request.QueryString["vcampo"]));
                ddlCriterio.SelectedValue = Convert.ToString(vc.VLC_INCLUSION_EXCLUSION);
                if (vc.VLC_INCLUSION_EXCLUSION == BusinessLayer.Mensajeria.ValidacionCampoBL.obtenerCriterioAplicacionInclusion())
                {
                    //Inclusión - Exclusión
                    if (vc.VLC_CONDICION != null)
                    {
                        //Condicion
                        ddlTipoRegla.SelectedValue = Convert.ToString(BusinessLayer.Mensajeria.ValidacionCampoBL.obtenerTipoReglaCondicion());
                        ddlCondicion.SelectedValue = Convert.ToString(vc.VLC_CONDICION);
                        txtValorComparacion.Text = Convert.ToString(vc.VLC_VALOR);
                    }
                    else
                    {
                        //Tabla de valores
                        ddlTipoRegla.SelectedValue = Convert.ToString(BusinessLayer.Mensajeria.ValidacionCampoBL.obtenerTipoReglaTablaValores());
                        ddlNombreTabla.SelectedValue = Convert.ToString(vc.TABLA.TBL_CODIGO);
                        ddlCampoTabla.SelectedValue = Convert.ToString(vc.COLUMNA.COL_CODIGO);
                    }
                }
                else
                {
                    //Especial
                    txtProcedure.Text = vc.VLC_PROCEDIMIENTO;
                }
            }
        }
        ...
        protected void btnAceptar_Click(object sender, EventArgs e)
        {
            ...
            vcampo.VLC_CODIGO = Convert.ToInt32(Request.QueryString["vcampo"]);
            ...
            BusinessLayer.Mensajeria.ValidacionCampoBL.modificarValidacionCampo(vcampo);
